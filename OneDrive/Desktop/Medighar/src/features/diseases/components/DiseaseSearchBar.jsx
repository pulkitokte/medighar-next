import { Search } from "lucide-react";

function DiseaseSearchBar({
  value,
  onChange,
  placeholder = "Search diseases by name or symptom...",
}) {
  return (
    <div className="transition-premium flex w-full items-center gap-3 rounded-full border border-slate-200 bg-white px-4 py-2 shadow-sm hover:shadow-md focus-within:border-amber-300 focus-within:shadow-md">
      <Search className="h-5 w-5 shrink-0 text-slate-400" aria-hidden="true" />
      <input
        type="text"
        value={value}
        onChange={(event) => onChange(event.target.value)}
        aria-label="Search diseases"
        placeholder={placeholder}
        className="h-11 w-full min-w-0 bg-transparent text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none sm:text-base"
      />
    </div>
  );
}

export default DiseaseSearchBar;